...
extend(MapnikRendererPool.prototype, {
    ...
    initialize : function(options) {
        ...
        this._cache.on("expired", function(key, pool) {
            pool.drain(function() {
                pool.destroyAllNow();
            });
        });
    ...
    _getRendererPool : function(key) {
        var that = this;
        return ninvoke(that._cache, 'get', key).then(function(pool) {
            return pool || that._newRendererPool(key).then(function(pool) {
                return ninvoke(that._cache, 'set', key, pool)//
                .then(function() {
                    return pool;
                });
            });
