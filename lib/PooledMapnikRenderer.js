...
var MapnikRenderer = Mosaic.Class.extend({
    ...
    getTileBufferSize : function() {
        var options = this.options;
        var config = options.config || {};
        return config.buffer_size || config.bufferBize || options.buffer_size
                || options.bufferBize || 128;
    },
    ...
    exec : function(action) {
        ...
        return Mosaic.P.then(function() {
            ...
            .then(function(m) {
                ...
                var executor = new MapnikRenderer.Executor(options);
                ...
                if (map) {
                    console.log('[ERROR] Releasing map ', err);
                    that._pool.release(map);
                }
                throw err;
            });
...
MapnikRenderer.Executor = Mosaic.Class.extend({
    ...
    getTileBufferSize : function() {
        return this.options.renderer.getTileBufferSize();
    },
    ...
    _prepareMap : function(options) {
        ...
        var bufferSize = this.getTileBufferSize();
        var map = this.getMap();
