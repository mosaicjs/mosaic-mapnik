...
var MapnikRenderer = Mosaic.Class.extend({
     ...
     * @param options.base
     *                base path to the XML; used to resolve relative references
     *                (ex: references to images etc)
     * @param options.tmpDir
     *                path to a temporary directory where generated files should
     *                be stored.
     */
    ...
    exec : function(action) {
        ...
        return Mosaic.P.then(function() {
            var map;
            return Mosaic.P.ninvoke(that._pool, that._pool.acquire).then(
                    function(m) {
                        map = m;
     ...
     * 
     * @param options.size.width
     *                width of the map
     * @param options.size.height
     *                height of the map
     * @param options.bbox
     *                bounding box [w, s, e, n]
     * @param options.xml
     *                a string-serialized Mapnik XML configuration
     */
    _newMapnikInstance : function(options) {
        ...
        return Mosaic.P.then(function() {
            ...
            map.bufferSize = options.bufferSize || 32;
...
MapnikRenderer.Executor = Mosaic.Class.extend({
    ...
    renderMap : function(options) {
        ...
        return Mosaic.P.then(function() {
            ...
            return that.withFile(options.file,
                    that.renderToFile.bind(that, options)).then(function() {
    ...
    renderTile : function(options) {
        ...
        return Mosaic.P.then(function() {
            ...
            return Mosaic.P.ninvoke(map, map.render, image, options).then(
                    function(img) {
                        var view = img.view(0, 0, tileSize, tileSize);
                        return Mosaic.P.ninvoke(view, view.encode, format,
                                options);
                    });
        });
    },
