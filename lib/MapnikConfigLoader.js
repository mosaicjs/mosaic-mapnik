...
MapnikConfigLoader.prototype = {
    ...
    readProjects : function(options, optionsList) {
        ...
        return Promise.all(optionsList.map(function(opt) {
            return that._resolveMmlReferences(opt);
    ...
    _mergeProjects : function(options, list) {
        ...
        options.mml = options.mml || {
            "srs" : "+init=epsg:4326"
        };
        this._mergeProjectInteractivity(options.mml, configs);
        this._mergeProjectStyles(options.mml, configs);
        this._mergeProjectDataLayers(options.mml, configs);
        return options;
    },

    _mergeProjectInteractivity : function(target, sources) {
        sources.forEach(function(source) {
            if (source.interactivity && !target.interactivity) {
                target.interactivity = source.interactivity;
            }
        });
        return target;
    },

    _mergeProjectStyles : function(target, sources) {
        target.Stylesheet = target.Stylesheet || [];
        (sources || []).forEach(function(source) {
            if (!source.Stylesheet)
                return;
            source.Stylesheet.forEach(function(style) {
    ...
    _mergeProjectDataLayers : function(target, sources) {
        target.Layer = target.Layer || [];
        var index = {};
        (sources || []).forEach(function(source) {
            if (!source.Layer)
                return;
            source.Layer.forEach(function(layer) {
